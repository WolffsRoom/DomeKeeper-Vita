...
bad_path = r'C:\Users\wolff\Documents\SDKVita\Dome Keeper.v5.0.4\GOGVersion\VitaBuild\.import\de_DE.png-c7076acc0ec75c0ee902f8937843f6fa.stex'

with open(bad_path, 'rb') as f:
    b = f.read()

# The Godot GDST format (StreamTexture) stores WebP as:
# header(28 bytes): GDST + w(u32) + h(u32) + flags(u32) + mip_limit(u32) + fmt(u32) + data_size(u32)
# data: 'WEBP'(4) + webp_payload_size(u32) + webp_payload_bytes
#
# In the bad file, the WebP blob at offset 28 starts with 'WEBP' but then has another 'RIFF' 
# making it 'WEBP' + 'RIFF' + ... which causes the Vita decoder to read 'RIFF' as the size
# field (= 1380206150), attempting a huge alloc -> !mem -> crash.
#
# The correct structure is:
#   b[28:32] = 'WEBP'  (magic)
#   b[32:36] = uint32 size of the actual RIFF WebP bytes that follow
#   b[36:]   = RIFF WebP data (starting with 'RIFF')

webp_blob = b[28:]
print(f"webp_blob[0:4] = {webp_blob[:4]}")      # Should be WEBP
print(f"webp_blob[4:8] = {webp_blob[4:8]}")      # Bad: 'RIFF' instead of a size

# The real RIFF WebP starts at offset 4 of webp_blob
real_riff_webp = webp_blob[4:]   # This is the actual RIFF...WEBP... data
print(f"real_riff_webp[:4] = {real_riff_webp[:4]}")  # Should be 'RIFF'
riff_inner_size = struct.unpack_from('<I', real_riff_webp, 4)[0]
print(f"RIFF inner size = {riff_inner_size}, real_riff_webp total = {len(real_riff_webp)}")

# Rebuild: keep the 28-byte GDST header, then write correct WEBP blob
new_webp_payload = real_riff_webp               # The raw RIFF WebP bytes
new_data_size = 4 + 4 + len(new_webp_payload)  # 'WEBP' + size_field + payload

new_stex = bytearray(b[:28])
# Update data_size field at offset 24
struct.pack_into('<I', new_stex, 24, new_data_size)
# Append: WEBP magic + payload size + payload
new_stex += b'WEBP'
new_stex += struct.pack('<I', len(new_webp_payload))
new_stex += new_webp_payload

print(f"Original size: {len(b)}, New size: {len(new_stex)}")
print(f"new_stex[28:32] = {bytes(new_stex[28:32])}  (should be WEBP)")
size_check = struct.unpack_from('<I', new_stex, 32)[0]
print(f"new_stex[32:36] = {size_check}  (should be {len(new_webp_payload)})")
print(f"new_stex[36:40] = {bytes(new_stex[36:40])}  (should be RIFF)")
