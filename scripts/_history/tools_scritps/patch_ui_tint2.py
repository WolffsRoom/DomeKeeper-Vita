...
content = re.sub(r'\t\t\telif node is PanelContainer.*?node\.add_to_group\("styled"\)', new_ui_block, content, flags=re.DOTALL)

with open('systems/style/Style.gd', 'w', encoding='utf-8') as f:
    f.write(content)

print("Style.gd UI elements modified to use modulate tinting!")
